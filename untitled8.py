...
Original file is located at
    https://colab.research.google.com/drive/1hCxOk_pWwbJ33ZqaGClwJCs9xMyCcSUt
"""
...
# Read the image.
sig_org = cv2.imread('/content/cv2/Mod2/Applications/extracted_sig.png', cv2.IMREAD_COLOR)

# Display the actual image in the browser.
Image('/content/cv2/Mod2/Applications/extracted_sig.png', width = '400')

# Display the image using imshow() so we can see the size with axis.
plt.imshow(sig_org[:, :, ::-1])
plt.title('Sample Signature');

sig = sig_org[200:, 100:3500, :]
plt.imshow(sig[:, :, ::-1]);
